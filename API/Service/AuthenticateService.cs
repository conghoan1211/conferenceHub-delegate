...
namespace API.Services
{
    ...
    public class AuthenticateService : IAuthenticateService
    {
        ...
        private readonly DelegateManagementContext _context;
        private readonly JwtAuthentication _jwtAuthen;

        public AuthenticateService(IMapper mapper, DelegateManagementContext context, JwtAuthentication jwtAuthen)
        {
            _mapper = mapper;
            _context = context;
        ...
        {
            string msg = "";
            var user = await _context.Users.FirstOrDefaultAsync(x => x.FullName == userLogin.Username || x.Email == userLogin.Username);
        ...
        public async Task<(string, LoginResult?)> LoginByGoogle(GoogleUserInfo input, HttpContext httpContext)
        {
            ...
            if (user == null)
            {
                ...
                user = new User
                {
                    UserId = userid,
                    GoogleId = input.Id,
                    FullName = input.Name ?? $"user_{userid.Substring(0, 8)}",
